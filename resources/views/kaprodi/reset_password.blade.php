{{-- resources/views/kaprodi/reset_password.blade.php --}}

@extends('layouts.auth_layout')

@section('title', 'Atur Ulang Sandi Kaprodi - SIPRAKTA')
...
<div class="reset-container">
    ...
    <form method="POST" action="{{ route('kaprodi.password.reset') }}">
        @csrf
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if (session('status'))
            window.showSuccessModal("{{ session('status') }}", "{{ route('kaprodi.password.reset.success') }}");
        @endif
        @if (session('error'))
