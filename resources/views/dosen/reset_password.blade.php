{{-- resources/views/dosen/reset_password.blade.php --}}

@extends('layouts.auth_layout')

@section('title', 'Atur Ulang Sandi Dosen - SIPRAKTA')
...
<div class="reset-container">
    ...
    <form method="POST" action="{{ route('dosen.password.reset') }}">
        @csrf
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if (session('status'))
            window.showSuccessModal("{{ session('status') }}", "{{ route('dosen.password.reset.success') }}");
        @endif
        @if (session('error'))
