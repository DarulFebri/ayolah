...
class KaprodiAuthController extends Controller
{
    /**
     * Menampilkan form login kaprodi.
     */
    public function loginForm()
    {
        return view('kaprodi.auth.login');
    }

    /**
     * Memproses percobaan login kaprodi.
     */
    ...
    {
        return view('kaprodi.forgot_password');
    }

    /**
     * Mengirim OTP ke email kaprodi untuk proses reset password.
    ...
    public function sendResetOtp(Request $request)
    {
        $request->validate([
            ...
            'email.exists' => 'Email ini tidak terdaftar sebagai email kaprodi.',
        ]);
        ...
        if (! $user) {
            return back()->withErrors([
                'email' => 'Email ini tidak terdaftar sebagai email kaprodi.',
            ])->withInput($request->only('email'));
        }
        ...
        return redirect()->route('kaprodi.otp.verify.form', ['email' => $user->email])
    ...
    public function showOtpVerifyForm(Request $request)
    {
        if (! $request->has('email')) {
            return redirect()->route('kaprodi.forgot.password.form')->with('error', 'Akses tidak sah. Email tidak ditemukan.');
        }

        return view('kaprodi.otp_verify', ['email' => $request->email]);
    }
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if ($user->otp === $request->otp && Carbon::now()->lessThan($user->otp_expires_at)) {
            ...
            return redirect()->route('kaprodi.password.reset.form', ['token' => $resetToken, 'email' => $user->email])
    ...
    public function resendOtp(Request $request)
    {
        ...
        if (! $user) {
            return back()->withErrors(['email' => 'Email kaprodi tidak ditemukan atau tidak terhubung ke akun pengguna.'])->withInput($request->only('email'));
        }
    ...
    public function showResetPasswordForm(Request $request)
    {
        ...
        if (! $token || ! $email) {
            return redirect()->route('kaprodi.forgot.password.form')
        ...
        return view('kaprodi.reset_password', compact('email', 'token'));
    }
    ...
    public function resetPassword(Request $request)
    {
        ...
        Log::info('Reset Password Request (Kaprodi):', $request->all());
        ...
        if (! $user) {
            Log::warning('Reset Password (Kaprodi): User not found or token invalid', ['email' => $request->email, 'token' => $request->token]);
        ...
        Log::info('Reset Password (Kaprodi): User found. ID:', ['id' => $user->id, 'email' => $user->email]);
        ...
        try {
            $user->save();
            Log::info('Reset Password (Kaprodi): Password updated successfully for user ID:', ['id' => $user->id]);
        } catch (\Exception $e) {
            Log::error('Reset Password (Kaprodi): Failed to save password for user ID: '.$user->id.' Error: '.$e->getMessage());

            return back()->withErrors(['password' => 'Terjadi kesalahan saat menyimpan password baru. Silakan coba lagi.']);
        }

        return redirect()->route('kaprodi.password.reset.success')->with('success', 'Password Anda berhasil diubah. Silakan login dengan password baru Anda.');
    }
    ...
    {
        return view('kaprodi.password_reset_success');
    }
}
