...
class AdminAuthController extends Controller
{
    ...
    /**
     * Mengirim OTP ke email admin untuk proses reset password.
    ...
    public function sendResetOtp(Request $request)
    {
        $request->validate([
            ...
            'email.exists' => 'Email ini tidak terdaftar sebagai email admin.',
        ]);
        ...
        if (! $user) {
            return back()->withErrors([
                'email' => 'Email ini tidak terdaftar sebagai email admin.',
            ])->withInput($request->only('email'));
        }
        ...
        return redirect()->route('admin.otp.verify.form', ['email' => $user->email])
    ...
    public function showOtpVerifyForm(Request $request)
    {
        if (! $request->has('email')) {
            return redirect()->route('admin.forgot.password.form')->with('error', 'Akses tidak sah. Email tidak ditemukan.');
        }

        return view('admin.otp_verify', ['email' => $request->email]);
    }
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if ($user->otp === $request->otp && Carbon::now()->lessThan($user->otp_expires_at)) {
            ...
            return redirect()->route('admin.password.reset.form', ['token' => $resetToken, 'email' => $user->email])
    ...
    public function resendOtp(Request $request)
    {
        ...
        if (! $user) {
            return back()->withErrors(['email' => 'Email admin tidak ditemukan atau tidak terhubung ke akun pengguna.'])->withInput($request->only('email'));
        }
    ...
    public function resetPassword(Request $request)
    {
        ...
        Log::info('Reset Password Request (Admin):', $request->all());
        ...
        if (! $user) {
            Log::warning('Reset Password (Admin): User not found or token invalid', ['email' => $request->email, 'token' => $request->token]);
        ...
        Log::info('Reset Password (Admin): User found. ID:', ['id' => $user->id, 'email' => $user->email]);
        ...
        try {
            $user->save();
            Log::info('Reset Password (Admin): Password updated successfully for user ID:', ['id' => $user->id]);
        } catch (\Exception $e) {
            Log::error('Reset Password (Admin): Failed to save password for user ID: '.$user->id.' Error: '.$e->getMessage());

            return back()->withErrors(['password' => 'Terjadi kesalahan saat menyimpan password baru. Silakan coba lagi.']);
        }

        return redirect()->route('admin.password.reset.success')->with('success', 'Password Anda berhasil diubah. Silakan login dengan password baru Anda.');
    }
    ...
    {
        return view('admin.password_reset_success');
    }
}
